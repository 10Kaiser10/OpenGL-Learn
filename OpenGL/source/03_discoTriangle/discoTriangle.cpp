...
int main(void)
{
    ...
    //shader stuff
    std::string vertPath = "../../../source/03_discoTriangle/vertex.shader";
    std::string fragPath = "../../../source/03_discoTriangle/fragment.shader";

    unsigned int shaderProg = createShaderProgram(vertPath, fragPath);
    glUseProgram(shaderProg);

    //uniform related
    int colorUniformLoc = glGetUniformLocation(shaderProg, "ourColor");
    float greenVal, currTime;
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        greenVal = (sin(currTime) / 2.0) + 0.5;
        glUniform4f(colorUniformLoc, 0.5, greenVal, 0.2, 1.0);
